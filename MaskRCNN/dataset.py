...
class COCODetection(object):
    ...
    def __init__(self, basedir, name):
        ...
        if not os.path.isdir(self._imgdir):
            logger.warn(f'COCODetection: falling back to coco_{name} because {self._imgdir} is not a dir')
            self._imgdir = os.path.realpath(os.path.join(basedir, f'coco_{name}'))

        logger.info(f'COCODetection: image folder {name}')
        assert os.path.isdir(self._imgdir), self._imgdir
        annotation_file = os.path.join(
            basedir, 'annotations/instances_{}.json'.format(name.replace('coco_', '')))
    ...
    def _add_detection_gt_orig(self, img, add_mask):
        """
        Add 'boxes', 'class', 'is_crowd' of this image to the dict, used by detection.
        If add_mask is True, also add 'segmentation' in coco poly format.
        """
        ...
        for objid, obj in enumerate(objs):
            ...
            if obj['area'] > 1 and w > 0 and h > 0 and w * h >= 4:
                all_boxes.append([x1, y1, x2, y2])
                # all_cls.append(self.COCO_id_to_category_id.get(obj['category_id'], obj['category_id']))
                all_boxes.append(obj['category_id'])
                iscrowd = obj.get("iscrowd", 0)
                all_iscrowd.append(iscrowd)
...
if __name__ == '__main__':
    basedir = '/Users/tadas/temp/gasefis/data'
    c = COCODetection(basedir, 'coco_ballets')
